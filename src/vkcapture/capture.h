...
/*
 * VENDORED from nowrep/obs-vkcapture (src/capture.h), GPLv2.
 * This is the wire protocol shared between the Vulkan capture LAYER (which lives
 * inside CS2 and produces frames) and the CONSUMER (our vkcapture-consumer, which
 * binds the socket and encodes). We only need the struct/define part of the
 * header — the function prototypes below the structs are the layer-side API and
 * are intentionally omitted here (the consumer never calls them).
 *
 * Keep this byte-exact with upstream: the structs are packed and exchanged raw
 * over the unix socket. If upstream bumps the protocol, re-vendor this file.
 */
